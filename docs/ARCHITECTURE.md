실제 폴더 구조와 문서를 비교해 보니, `01_app/`에 `display_test/` 컴포넌트가 추가되었지만 문서에는 반영되지 않았습니다. 문서의 '컴포넌트 상세' 섹션에도 `display_test`를 추가해야 합니다.

현재 문서의 '## 컴포넌트 폴더 구조' 섹션은 실제 폴더와 거의 일치하지만, `01_app/`에 `display_test/`가 빠져 있습니다.
...
| display/pages/TallyPage | Tally 상태 표시 페이지 | DisplayManager | 🚧 |
| display/pages/InfoPage | 정보 표시 페이지 (IP, 배터리 등) | DisplayManager | 🚧 |
...
| TallyPage | Tally 상태 | 채널 번호, PVW/PGM 상태, 프로그램 이름 |
| InfoPage | 정보 표시 | IP 주소, 배터리, LoRa RSSI, 연결 상태 |
